...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'POST' || req.method === 'PUT') {
    ...
    try {
      ...
      if (req.method === 'POST') {
        // insert work_plan (ลบ sort_order ออก)
        const [result]: any = await connection.execute(
          'INSERT INTO work_plans (job_code, job_name, start_time, end_time, production_date) VALUES (?, ?, ?, ?, ?)',
          [job_code, job_name, start_time, end_time, date]
        );
